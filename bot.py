...
async def schichtwechsel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    await update_schichtwechsel_menu()

# Command: VoiceMaster Eingang festlegen
...
# Command: Rollen für Auswahl festlegen/hinzufügen
...
# Command: Rollen wieder entfernen
...
# Menü im Wechselchannel posten/aktualisieren
def make_schichtwechsel_embed_and_view(guild):
    embed = discord.Embed(
        title="🕒 Schichtübergabe",
        description="Wähle einen Nutzer, an den du die Schicht übergeben willst:",
    ...
    # Discord limitiert Dropdown auf 25. Ab hier: QuickSearch/Autocomplete wäre ein Powerfeature!
    options = options[:25]
    sel = discord.ui.Select(placeholder="Nutzer wählen...", options=options, max_values=1)
    async def sel_cb(inter):
        selected_id = int(inter.data["values"][0])
        anfragender = inter.user
        zieluser = guild.get_member(selected_id)
        # Check 1: Anfragender muss im VoiceChannel sein
        if not anfragender.voice or not anfragender.voice.channel:
            return await inter.response.send_message("❌ Du musst in einem Sprachkanal sein für Schichtwechsel.", ephemeral=True)
        # Check 2: Zielnutzer da?
        if not zieluser:
            return await inter.response.send_message("❌ Nutzer ist nicht mehr auf dem Server.", ephemeral=True)
        # Check 3: Ist der Nutzer online?
        if not zieluser.status in [discord.Status.online, discord.Status.idle, discord.Status.dnd]:
            try:
                await zieluser.send(f"{anfragender.mention} möchte Schicht übergeben! Bitte melde dich asap für den Schichtwechsel.")
                await inter.response.send_message(f"{zieluser.mention} ist offline. Er wurde per DM benachrichtigt.", ephemeral=True)
            except:
                await inter.response.send_message(f"{zieluser.mention} ist offline und hat DMs deaktiviert. Bitte kontaktiere ihn manuell.", ephemeral=True)
            return
        # Move 1: Anfragender in VoiceMaster Eingang
        voice_kanal = guild.get_channel(schicht_cfg["voice_channel_id"])
        if not voice_kanal:
            return await inter.response.send_message("❌ VoiceMaster-Eingangskanal nicht gefunden!", ephemeral=True)
        try:
            await anfragender.move_to(voice_kanal)
            await inter.response.send_message("Du wurdest in den VoiceMaster-Eingang verschoben. Starte Schichtübergabe...", ephemeral=True)
        except Exception as e:
            return await inter.response.send_message("❌ Verschieben fehlgeschlagen (Prüfe Rechte)!", ephemeral=True)
        await asyncio.sleep(5)  # Zeit für VoiceMaster Bot
        # Jetzt Channel erneut suchen (könnte sich geändert haben)
        new_channel = anfragender.voice.channel if anfragender.voice else None
        if not new_channel or new_channel.id == voice_kanal.id:
            return await inter.followup.send("❌ Temporärer Channel wurde nicht erkannt! Bitte manuell checken.", ephemeral=True)
        try:
            await zieluser.move_to(new_channel)
            await inter.followup.send(f"{zieluser.mention} wurde zu dir verschoben! Schichtübergabe läuft.", ephemeral=True)
            # Log, falls Channel gesetzt
            log_id = schicht_cfg.get("log_channel_id")
            if log_id:
                log_ch = guild.get_channel(log_id)
                if log_ch:
                    await log_ch.send(
                        f"**Schichtwechsel**: {anfragender.mention} ➡️ {zieluser.mention}\n"
                        f"Channel: `{new_channel.name}`\nZeit: <t:{int(discord.utils.utcnow().timestamp())}:F>")
        except Exception:
            await inter.followup.send(f"{zieluser.mention} konnte nicht verschoben werden (Prüfe Rechte/DND/AFK).", ephemeral=True)
    sel.callback = sel_cb
    view.add_item(sel)
    return embed, view

async def update_schichtwechsel_menu():
    ...
    try:
        ...
        async for m in ch.history(limit=20):
            if m.author == bot.user and m.components:
                msg = m
                break
        embed, view = make_schichtwechsel_embed_and_view(ch.guild)
        if msg:
            await msg.edit(embed=embed, view=view)
        else:
            await ch.send(embed=embed, view=view)
    except Exception as e:
        print("Schichtmenü Fehler:", e)

# Optional: Beim Start menü updaten
@bot.event
async def on_ready():
